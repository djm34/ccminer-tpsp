...
__constant__ uint32_t pTarget2[8]; // 32 bytes
...
__device__ __forceinline__
static bool hashbelowtarget2(const uint32_t *const __restrict__ hash, const uint32_t *const __restrict__ target)
{
bool res = false;

	if (((uint64_t*)hash)[3] <= ((uint64_t*)target)[3])
	res=true;
...
void cuda_checkhash_64(uint32_t threads, uint32_t startNounce, uint32_t *hash, uint32_t *resNonces)
{
	...
	if (thread < threads)
	{
		...
		if (resNonces[0] == UINT32_MAX) {
			if (hashbelowtarget2(inpHash, pTarget))
